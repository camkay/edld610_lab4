...
ggplot(honey_data, aes(x = year, y = totalprod_mil, color = state)) +
  ...
  gghighlight(state == "OR" |
                state == "WA" |
                state == "CA") +
  scale_color_carto_d(palette = "BurgYl") +
  theme_minimal() +
...
Note that the line representing California may not be the best choice if we want the plot to be readable to all viewers (it's hard to see in the desaturated condition).
...
While the above plot (Fig. 2) uses a color blind safe pallette, note that the line representing Washington is not easily viewable in the Desaturated condition.
