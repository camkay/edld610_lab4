...
ggplot(honey_data, aes(x = year, y = totalprod_mil, color = state)) +
  ...
  gghighlight(state == "OR" |
                state == "WA" |
                state == "CA") +
  scale_color_manual(values = c("#fde0dd", "#fa9fb5", "#c51b8a")) +
  theme_minimal() +
...
**Note that the line representing California may not be the best choice if we want the plot to be readable to all viewers.**
...
**While the following plot uses a color blind safe pallette, note that the line representing Washington is not easily viewable in the Desaturated condition.**
